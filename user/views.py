...
def myevent(request):
    ...
    sdata=request.GET.get('search')#Arjeet Singh
    if cid is not None:
...
def logout(request):
    user=request.session.get('user')#ram@gmial.com
    if user:
...
def booking(request):
    user = request.session.get('user')  # ram@gmail.com
    if user:
        ...
        booknow(userid=user, event_name=title, event_picture=epic, speaker_name=sname, city=city, hotel=hotel,
                event_date=edate, event_price=price, speaker_picture=spic, booking_date=datetime.now().date()).save()
        return HttpResponse(
            "<script>alert('Your event is booked successfully..');location.href='/user/event/'</script>")
...
def myticket(request):
    ...
    if bid is not None:
        ...
        return HttpResponse("<script>alert('Your ticket has been canceled..');location.href='/user/myticket/'</script>")
